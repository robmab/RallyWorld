 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'Fitness_Hub_Gallery' ) ) {

    class Fitness_Hub_Gallery extends WP_Widget {
        ...
        function __construct() {
            parent::__construct(
                    ...
                    esc_html__('AT Gallery Section', 'fitness-hub'),
                    /*Widget description*/
                    array(
                            'description' => esc_html__( 'Show Gallery Section.', 'fitness-hub' )
                    )
            );
        }
        ...
        public function form( $instance ) {
            ...
            <br/>
            <small><?php esc_html_e( 'Add Page, Reorder and Remove. Please do not forget to add Featured Image on selected pages. ', 'fitness-hub' ); ?></small>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'image_popup_type' ); ?>"><?php esc_html_e( 'Image Popup Type', 'fitness-hub' ); ?></label>
        ...
        public function update( $new_instance, $old_instance ) {
	        ...
	        $fitness_hub_image_sizes              = fitness_hub_get_image_sizes_options();
	        $instance[ 'fitness_hub_img_size' ]   = fitness_hub_sanitize_choice_options( $new_instance[ 'fitness_hub_img_size' ], $fitness_hub_image_sizes, 'large' );

	        $fitness_hub_gallery_image_popup      = fitness_hub_gallery_image_popup();
	        $instance[ 'image_popup_type' ]         = fitness_hub_sanitize_choice_options( $new_instance[ 'image_popup_type' ], $fitness_hub_gallery_image_popup, 'gallery' );
            ...
            <section id="<?php echo $unique_id;?>" class="at-widgets acme-gallery">
                <div class="full-width-container">
                    ...
                    if( !empty ( $post_in ) && is_array( $post_in )) :
	                    ...
	                    $gallery_query = new WP_Query( $fitness_hub_post_in_page_args );
                        ?>
                        <div class="row fullwidth-row">
                            ...
                            if ( $gallery_query->have_posts() ):
                                ...
                                while( $gallery_query->have_posts() ):$gallery_query->the_post();
	                                if( 1 == $column_number ){
		                                $fitness_hub_column = " col-xs-12";
	                                }
                                    elseif( 2 == $column_number ){
		                                $fitness_hub_column = " col-sm-6";
	                                }
                                    elseif( 3 == $column_number ){
		                                $fitness_hub_column = " col-sm-4 col-md-4";
	                                }
                                    elseif( 4 == $column_number ){
		                                $fitness_hub_column = ' col-sm-3 col-md-3';
	                                }
	                                else{
		                                $fitness_hub_column = " col-sm-3 col-md-3";
	                                }
                                    ?>
                                    <div class="at-gallery-item <?php echo esc_attr( $fitness_hub_column ); ?>">
                                        <div class="gallery-inner-item <?php echo esc_attr( $animation );?>">
                                            ...
                                            if( has_post_thumbnail() ):
                                                $image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), $fitness_hub_img_size );
                                            else:
    ...
    } // Class Fitness_Hub_Gallery ends here
}