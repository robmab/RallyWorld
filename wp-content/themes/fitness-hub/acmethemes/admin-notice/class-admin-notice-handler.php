 ...
 *
 * Class Fitness_Hub_Notice_Handler
 */
class Fitness_Hub_Notice_Handler {
	...
	public function replace_term_ids( $replace_term_ids ){
		...
		/*Terms IDS*/
		$term_ids = array(
            'fitness-hub-exclude-categories',
            'fitness_hub_post_cat',
            'fitness_hub_post_tag',
            'fitness_hub_cat_id',
            'fitness_hub_wc_product_cat',
            'fitness_hub_wc_product_tag',
	...
	public function admin_notice() {
		...
		if ( ! $notice_nag ) {
			wp_enqueue_style( 'fitness-hub-notice', get_template_directory_uri() . '/acmethemes/admin-notice/admin-notice.css', array(), '3.0.0' );
			wp_enqueue_script( 'fitness-hub-adi-install', get_template_directory_uri()  . '/acmethemes/admin-notice/admin-notice.js', array( 'jquery' ), '', true );

			$translation = array(
			        'btn_text' => esc_html__( 'Processing...', 'fitness-hub' ),
                    'nonce'    => wp_create_nonce( 'fitness-hub-demo-import-nonce' ),
                    'adminurl'    => admin_url()
            );
			wp_localize_script( 'fitness-hub-adi-install', 'fitness_hub_adi_install', $translation );
	...
	public static function hide_notices() {
		...
		if ( isset( $_GET['at-gsm-hide-notice'] ) && isset( $_GET['at_gsm_admin_notice_nonce'] ) ) { // WPCS: input var ok.
			if ( ! wp_verify_nonce( wp_unslash( $_GET['at_gsm_admin_notice_nonce'] ), 'at_gsm_hide_notices_nonce' ) ) { // phpcs:ignore WordPress.VIP.ValidatedSanitizedInput.InputNotSanitized
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'fitness-hub' ) ); // WPCS: xss ok.
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'fitness-hub' ) ); // WPCS: xss ok.
			}

			$notice_type = sanitize_text_field( wp_unslash( $_GET['at-gsm-hide-notice'] ) );
			update_option( 'fitness_hub_admin_notice_'. $notice_type, 1 );

			/*Update to Hide.*/
			if ( 'welcome' === $_GET['at-gsm-hide-notice'] ) {
				update_option( 'fitness_hub_admin_notice_'. $notice_type, 1 );
			} else { // Show.
				delete_option( 'fitness_hub_admin_notice_'. $notice_type );
			}
		}
	...
	public function getting_started() {
        ...
        <div id="at-gsm" class="updated notice-info at-gsm">
            <a class="at-gsm-close notice-dismiss" href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'activated' ), add_query_arg( 'at-gsm-hide-notice', 'welcome' ) ), 'at_gsm_hide_notices_nonce', 'at_gsm_admin_notice_nonce' ) ); ?>">
				<?php esc_html_e( 'Dismiss', 'fitness-hub' ); ?>
            </a>
            <div class="at-gsm-container">
                <img class="at-gsm-screenshot" src="<?php echo esc_url(get_template_directory_uri().'/screenshot.jpg' )?>" alt="<?php esc_attr_e( 'Fitness Hub', 'fitness-hub' ); ?>" />
                <div class="at-gsm-notice">
                    <h2>
						<?php
						printf(
						/* translators: 1: welcome page link starting html tag, 2: welcome page link ending html tag. */
							esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer, please make sure you visit our %2$swelcome page%3$s.', 'fitness-hub' ), '<strong>'. wp_get_theme()->get('Name'). '</strong>','<a href="' . esc_url( admin_url( 'themes.php?page=fitness-hub-info' ) ) . '">','</a>' );
						?>
                    </h2>

                    <p class="plugin-install-notice"><?php esc_html_e( 'Clicking the button below will install and activate the Advanced Import plugin.', 'fitness-hub' ); ?></p>

                    <a class="at-gsm-btn button button-primary button-hero" href="#" data-name="" data-slug="" aria-label="<?php esc_attr_e( 'Get started with Fitness Hub', 'fitness-hub' ); ?>">
		                <?php esc_html_e( 'Get started with Fitness Hub', 'fitness-hub' );?>
                    </a>
                </div>
            </div>
        </div>
	...
	public function install_advanced_import() {
		...
		check_ajax_referer( 'fitness-hub-demo-import-nonce', 'security' );
        ...
        if ( ! current_user_can( 'install_plugins' ) ) {
            $status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'fitness-hub' );
        ...
        if ( is_wp_error( $result ) ) {
            ...
            $status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'fitness-hub' );
	...
	public function setup_review_notice() {
		...
		// Set the installed time in `fitness_hub_theme_installed_time` option table.
		$option = get_option('fitness_hub_theme_installed_time' );

		if ( ! $option ) {
			update_option('fitness_hub_theme_installed_time', time() );
		}
	...
	public function display_review_notice() {
		 ...
		 */
		if ( ( get_option('fitness_hub_theme_installed_time' ) > strtotime( '-15 day' ) ) || ( $ignored_notice_partially > strtotime( '-15 day' ) ) || ( $ignored_notice ) ) {
			return;
		}
		?>
		<div class="notice updated at-review-notice">
			<p>
				...
				/* Translators: %1$s current user display name. */
					esc_html__(
						'Howdy, %1$s! It seems that you have been using this theme for more than 15 days. We hope you are happy with everything that the theme has to offer. If you can spare a minute, please help us by leaving a 5-star review on WordPress.org.  By spreading the love, we can continue to develop new amazing features in the future, for free!', 'fitness-hub'
					),
			...
			<div class="links">
				<a href="https://wordpress.org/support/theme/fitness-hub/reviews/?filter=5#new-post" class="btn button-primary" target="_blank">
					<span class="dashicons dashicons-thumbs-up"></span>
					<span><?php esc_html_e( 'Sure', 'fitness-hub' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice_partially=0" class="btn at-danger-btn">
					<span class="dashicons dashicons-calendar"></span>
					<span><?php esc_html_e( 'Maybe later', 'fitness-hub' ); ?></span>
				</a>

				<a href="?nag_remove_theme_review_notice=0" class="btn at-success-btn">
					<span class="dashicons dashicons-smiley"></span>
					<span><?php esc_html_e( 'I already did', 'fitness-hub' ); ?></span>
				</a>

				<a href="<?php echo esc_url( 'https://wordpress.org/support/theme/fitness-hub/' ); ?>" class="btn at-default-btn" target="_blank">
					<span class="dashicons dashicons-edit"></span>
					<span><?php esc_html_e( 'Got theme support question?', 'fitness-hub' ); ?></span>
				</a>
			</div>
	...
	public function remove_review_notice() {
		...
		global $current_user;
		$user_id                  = $current_user->ID;
		$theme_installed_time     = get_option('fitness_hub_theme_installed_time' );
		$ignored_notice           = get_user_meta( $user_id, 'remove_theme_review_notice', true );
		$ignored_notice_partially = get_user_meta( $user_id, 'nag_remove_theme_review_notice_partially', true );

		// Delete options data.
		if ( $theme_installed_time ) {
			delete_option('fitness_hub_theme_installed_time' );
		}
	...
	public function review_notice_enqueue() {
		...
		wp_enqueue_style( 'fitness-hub-review-notice', get_template_directory_uri()  . '/acmethemes/admin-notice/admin-notice.css' );
 ...
 * @since    1.0.0
 */
function fitness_hub_notice_handler( ) {
	return Fitness_Hub_Notice_Handler::instance();
}
fitness_hub_notice_handler()->run();


/*For this theme only*/
if( !function_exists( 'fitness_hub_ai_update_image_size') ){
	function fitness_hub_ai_update_image_size(){
		/*Thumbnail Size*/
...
add_action( 'advanced_import_before_content_screen', 'fitness_hub_ai_update_image_size' );