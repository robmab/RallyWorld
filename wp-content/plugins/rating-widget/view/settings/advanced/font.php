...
<table id="rw_font_settings" cellspacing="0">
	<tr id="rw_font_size" class="rw-odd">
		...
		<td>
			<select
				onchange="rwStar.setFontSize(this.value + 'px'); rwNero.setFontSize(this.value + 'px'); RWM.Code.refresh();">
				<?php
	...
	<tr id="rw_font_color" class="rw-even">
		<td>
			<span class="rw-ui-def"><?php _erw( 'color' ) ?>:</span>
		</td>
		<td>
	...
	<tr id="rw_font_hover_color" class="rw-odd">
		<td>
			<span class="rw-ui-def"><?php _erw( 'hover' ) ?>:</span>
		</td>
		<td>
	...
	<tr id="rw_font_type" class="rw-even">
		<td>
			<span class="rw-ui-def"><?php _erw( 'type' ) ?>:</span>
		</td>
		<td>
			<?php
				$fonts = array(
...
//                    "lucida" => 'lucida grande',
	...
	<tr id="rw_font_style" class="rw-odd">
		<td class="rw-ui-def-width">
			<span class="rw-ui-def"><?php _erw( 'style' ) ?>:</span>
		</td>
		<td>
			<div class="rw-ui-img-radio<?php if ( $font->bold ) {
				echo " rw-selected";
			} ?>" onclick="rwStar.toggleBold(); rwNero.toggleBold();">
				<i class="rw-ui-holder"><i id="rw_ui_bold" class="rw-ui-sprite rw-ui-large"></i></i>
				<span><b>Bold</b></span>
				<input type="checkbox" name="rw-font-bold" value="0"<?php if ( $font->bold ) {
					echo ' checked="checked"';
				} ?> />
			</div>
			<div class="rw-ui-img-radio<?php if ( $font->italic ) {
				echo " rw-selected";
			} ?>" onclick="rwStar.toggleItalic(); rwNero.toggleItalic();">
				<i class="rw-ui-holder"><i id="rw_ui_italic" class="rw-ui-sprite rw-ui-large"></i></i>
				<span><i>Italic</i></span>
				<input type="checkbox" name="rw-font-italic" value="1"<?php if ( $font->italic ) {
					echo ' checked="checked"';
				} ?> />
			</div>
		</td>
